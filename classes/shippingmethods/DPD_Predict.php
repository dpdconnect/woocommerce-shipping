...
class DPD_Predict extends \WC_Shipping_Method
{
    /**
    ...
    {
        $this->id                 = 'dpd_predict';
        $this->title              = 'DPD Predict';
        $this->instance_id        = absint($instance_id);
        $this->method_title       = __('DPD Predict', 'dpdconnect');
        $this->method_description = __('B2C Predict DPD shipment', 'dpdconnect');
        $this->supports           = [
            'shipping-zones',
            'settings',
            'instance-settings',
    ...
    {
        $this->instance_form_fields = include 'includes/settings-dpd-predict.php';

        $this->init_form_fields();
        $this->init_settings();

        $this->enabled              = $this->get_option('enabled');
        $this->title                = (empty($this->get_option('title'))) ? 'DPD Predict Shipment' : $this->get_option('title');
    ...
    public function init_form_fields()
    {
        $this->form_fields = [
            'enabled' => [
                ...
                'type' => 'checkbox',
                'description' => __('Enable DPD Predict.', 'dpdconnect'),
                'default' => 'yes'
            ],
            'title' => [
